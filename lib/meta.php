...
if ( ! class_exists( 'NgfbMeta' ) ) {
	...
	class NgfbMeta {
		...
		public function add_mod_column_headings( $columns, $mod_name = '' ) { 
			if ( ! empty( $mod_name ) ) {
				foreach ( array( 
					'schema_id' => sprintf( _x( '%s Schema', 'column title', 'nextgen-facebook' ), $this->p->cf['menu_label'] ),
					'og_img' => sprintf( _x( '%s Img', 'column title', 'nextgen-facebook' ), $this->p->cf['menu_label'] ),
					'og_desc' => sprintf( _x( '%s Desc', 'column title', 'nextgen-facebook' ), $this->p->cf['menu_label'] ),
				) as $key => $label ) {
		...
		protected function get_mod_column_content( $value, $column_name, $mod ) {
			...
			if ( strpos( $column_name, $lca.'_' ) !== 0 )	// example: ngfb_og_img
				return $value;
