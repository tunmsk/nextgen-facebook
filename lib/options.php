				...
				if ( ! $network ) {
					if ( ! $this->p->check->aop( $this->p->cf['lca'], 
						...
						foreach ( array(
							...
							if ( $opts[$idx] != $def_val ) {
								if ( is_admin() )
									$this->p->notice->warn( sprintf( __( 'Non-standard value found for the Free version \'%s\' option - resetting the option to its default value.', 'nextgen-facebook' ), $idx ), true );
								$opts[$idx] = $def_val;
