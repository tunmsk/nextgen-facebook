		...
		public function check_options( $options_name, &$opts = array(), $network = false, $activate = false ) {
			...
			if ( is_array( $opts ) && ! empty( $opts ) ) {	// just in case
				...
				if ( is_admin() ) {
					...
					$this->p->notice->err( $err_msg.' '.sprintf( __( 'The plugin settings have been returned to their default values &mdash; <a href="%s">please review and save the new settings</a>.', 'nextgen-facebook' ), $admin_url ) );
				}
				...
				if ( ! $activate && $prev_version !== $opts['options_version'] ) {
					...
					if ( is_admin() ) {
						$this->p->notice->inf( sprintf( __( 'Plugin settings (%s) have been upgraded and saved.',
							'nextgen-facebook' ), $options_name ), true, __FUNCTION__.'_upgraded', true );
					}
				} elseif ( $this->p->debug->enabled ) {
