...
if ( ! class_exists( 'NgfbGplAdminMeta' ) ) {
	...
	class NgfbGplAdminMeta {
		...
		public function filter_meta_media_rows( $table_rows, $form, $head, $mod ) {
			...
			if ( $mod['is_post'] )
				$table_rows[] = '<td colspan="2" align="center">'.
					$this->p->msgs->get( 'pro-about-msg', __( 'You can change the social image by selecting a featured image, attaching one or more images, or including images in the content.', 'nextgen-facebook' ).' '.__( 'The service API modules required for embedded video discovery are available in the Pro version.', 'nextgen-facebook' ) ).'</td>';
			...
			if ( ! SucomUtil::get_const( 'NGFB_SCHEMA_DISABLE' ) ) {
