	...
	class NgfbGplAdminPost {
		...
		public function filter_post_header_rows( $rows, $form, $head_info ) {
			...
			$this->p->util->get_th( _x( 'Article Topic',
				'option label', 'nextgen-facebook' ), 'medium', 'post-og_art_section' ).
			'<td class="blank">'.$this->p->options['og_art_section'].'</td>';

			$rows['og_title'] = $this->p->util->get_th( _x( 'Default Title',
				'option label', 'nextgen-facebook' ), 'medium', 'meta-og_title' ). 
			...
			$rows['og_desc'] = $this->p->util->get_th( _x( 'Default (Facebook / Open Graph, LinkedIn, Pinterest Rich Pin) Description',
				'option label', 'nextgen-facebook' ), 'medium', 'post-og_desc', $head_info ).	// $head_info required for 'ptn'
			( $post_status === 'auto-draft' ? $td_save_draft :
			...
			$rows['schema_desc'] = $this->p->util->get_th( _x( 'Google / Schema Description',
				'option label', 'nextgen-facebook' ), 'medium', 'meta-schema_desc' ).
			( $post_status === 'auto-draft' ? $td_save_draft :
			...
			$this->p->util->get_th( _x( 'Google Search / SEO Description',
				'option label', 'nextgen-facebook' ), 'medium', 'meta-seo_desc' ).
			( $post_status === 'auto-draft' ? $td_save_draft :
			...
			$rows['tc_desc'] = $this->p->util->get_th( _x( 'Twitter Card Description',
				'option label', 'nextgen-facebook' ), 'medium', 'meta-tc_desc' ).
			( $post_status === 'auto-draft' ? $td_save_draft :
			...
			$this->p->util->get_th( _x( 'Sharing URL',
				'option label', 'nextgen-facebook' ), 'medium', 'meta-sharing_url' ).
		...
		public function filter_post_media_rows( $rows, $form, $head_info ) {
			...
			$rows[] = '<td></td><td><h5>'.
				_x( 'Priority Image Information',
					'metabox title', 'nextgen-facebook' ).'</h5></td>';
			...
			$rows['og_img_id'] = $this->p->util->get_th( _x( 'Image ID',
				'option label', 'nextgen-facebook' ), 'medium', 'meta-og_img_id' ).
			'<td class="blank">&nbsp;</td>';

			$rows['og_img_url'] = $this->p->util->get_th( _x( 'or an Image URL',
				'option label', 'nextgen-facebook' ), 'medium', 'meta-og_img_url' ).
			'<td class="blank">&nbsp;</td>';

			$rows['og_img_max'] = '<tr class="hide_in_basic">'.
			$this->p->util->get_th( _x( 'Maximum Images',
				'option label', 'nextgen-facebook' ), 'medium', 'meta-og_img_max' ).
			...
			$rows['og_vid_embed'] = $this->p->util->get_th( _x( 'Video Embed HTML',
				'option label', 'nextgen-facebook' ), 'medium', 'meta-og_vid_embed' ).
			'<td class="blank">&nbsp;</td>';

			$rows['og_vid_url'] = $this->p->util->get_th( _x( 'or a Video URL',
				'option label', 'nextgen-facebook' ), 'medium', 'meta-og_vid_url' ).
			'<td class="blank">&nbsp;</td>';

			$rows['og_vid_title'] = '<tr class="hide_in_basic">'.
			$this->p->util->get_th( _x( 'Video Name / Title',
				'option label', 'nextgen-facebook' ), 'medium', 'meta-og_vid_title' ).
			'<td class="blank">&nbsp;</td>';

			$rows['og_vid_desc'] = '<tr class="hide_in_basic">'.
			$this->p->util->get_th( _x( 'Video Description',
				'option label', 'nextgen-facebook' ), 'medium', 'meta-og_vid_desc' ).
			'<td class="blank">&nbsp;</td>';
			...
			$this->p->util->get_th( _x( 'Maximum Videos',
				'option label', 'nextgen-facebook' ), 'medium', 'meta-og_vid_max' ).
			'<td class="blank">'.$this->p->options['og_vid_max'].'</td>';

			$rows['og_vid_prev_img'] = $this->p->util->get_th( _x( 'Include Preview Image(s)',
				'option label', 'nextgen-facebook' ), 'medium', 'meta-og_vid_prev_img' ).
			...
			if ( ! SucomUtil::get_const( 'NGFB_RICH_PIN_DISABLE' ) ) {
				...
				$this->p->util->get_th( _x( 'Image ID',
					'option label', 'nextgen-facebook' ), 'medium', 'meta-rp_img_id' ).
				'<td class="blank">&nbsp;</td>';
	
				$rows['rp_img_url'] = '<tr class="hide_in_basic">'.
				$this->p->util->get_th( _x( 'or an Image URL',
					'option label', 'nextgen-facebook' ), 'medium', 'meta-rp_img_url' ).
				'<td class="blank">&nbsp;</td>';
			}
