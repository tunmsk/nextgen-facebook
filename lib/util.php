	...
	class NgfbUtil extends SucomUtil {
		...
		public function get_topics() {
			...
			if ( ( $topics = file( NGFB_TOPICS_LIST, 
				...
				$this->p->notice->err( sprintf( __( 'Error reading the %s topic list file.', 'nextgen-facebook' ), NGFB_TOPICS_LIST ) );
				return $topics;
			}
			...
			switch ( $option_type ) {
				...
				case 'url':
					if ( $val !== '' ) {
						...
						if ( strpos( $val, '//' ) === false ) {
							$this->p->notice->err( sprintf( __( 'The value of option \'%s\' must be a URL - resetting the option to its default value.', 'nextgen-facebook' ), $key ), true );
							$val = $def_val;
						}
					}
					break;
				...
				case 'img_dim':		// image dimensions, subject to minimum value (typically, at least 200px)
					...
					elseif ( ! is_numeric( $val ) || $val < $min_int ) {
						$this->p->notice->err( sprintf( __( 'The value of option \'%s\' must be equal to or greather than %s - resetting the option to its default value.', 'nextgen-facebook' ), $key, $min_int ), true );
						$val = $def_val;
				...
				case 'blank_num':
					if ( $val !== '' ) {
						if ( ! is_numeric( $val ) ) {
							$this->p->notice->err( sprintf( __( 'The value of option \'%s\' must be numeric - resetting the option to its default value.', 'nextgen-facebook' ), $key ), true );
							$val = $def_val;
						} else $val = (int) $val;	// cast as integer
					}
					break;
				...
				case 'auth_id':
					...
					if ( $val !== '' && preg_match( '/[^A-Z0-9\-]/', $val ) ) {
						$this->p->notice->err( sprintf( __( '\'%1$s\' is not an acceptable value for option \'%2$s\' - resetting the option to its default value.', 'nextgen-facebook' ), $val, $key ), true );
						$val = $def_val;
				...
				case 'api_key':
					...
					if ( $val !== '' && preg_match( '/[^a-zA-Z0-9_]/', $val ) ) {
						$this->p->notice->err( sprintf( __( 'The value of option \'%s\' must be alpha-numeric - resetting the option to its default value.', 'nextgen-facebook' ), $key ), true );
						$val = $def_val;
					} else $val = (string) $val;		// cast as string
					break;
				...
				case 'html':
					if ( $val !== '' ) {
						$val = trim( $val );
						if ( ! preg_match( '/<.*>/', $val ) ) {
							$this->p->notice->err( sprintf( __( 'The value of option \'%s\' must be HTML code - resetting the option to its default value.', 'nextgen-facebook' ), $key ), true );
							$val = $def_val;
						}
					}
					break;
				...
				case 'not_blank':
					if ( $val === '' ) {
						$this->p->notice->err( sprintf( __( 'The value of option \'%s\' cannot be empty - resetting the option to its default value.', 'nextgen-facebook' ), $key ), true );
						$val = $def_val;
					}
					break;
		...
		public function get_head_meta( $url, $query = '/html/head/meta', $remove_self = false ) {
			...
			if ( class_exists( 'DOMDocument' ) ) {
				...
				if ( is_admin() )
					$this->p->notice->err( sprintf( __( 'The DOMDocument PHP class is missing - unable to read the head meta from %s. Please contact your hosting provider to install the missing DOMDocument PHP class.', 'nextgen-facebook' ), $url ), true );
			}
			return $ret;
		}
		...
		public static function save_time( $lca, $version, $type, $protect = false ) {
			if ( ! is_bool( $protect ) ) {
				if ( ! empty( $protect ) ) {
					if ( ( $ts_version = SucomUtil::get_option_key( NGFB_TS_NAME, $lca.'_'.$type.'_version' ) ) !== null &&
		...
		public function add_image_wh( $keys, &$opts ) {
			if ( SucomUtil::get_const( 'NGFB_GETIMGSIZE_DISABLE' ) )
				return $opts;
		...
		public function json_format( $json, $options = JSON_UNESCAPED_SLASHES, $depth = 512 ) {
			...
			if ( $json_format ) {
				$classname = NgfbConfig::load_lib( false, 'ext/jsonformat', 'suextjsonformat' );
