		...
		public function get_head_meta( $request, $query = '/html/head/meta', $remove_self = false ) {
			...
			if ( $remove_self && strpos( $html, $lca.' meta tags begin' ) !== false ) {	// quick check
				...
				if ( ! $count ) {
					if ( is_admin() ) {
						$short = $this->p->cf['plugin'][$lca]['short'];
						$this->p->notice->err( sprintf( __( 'The PHP preg_replace() function failed to remove the %1$s meta tag block &mdash; this could be an indication of a problem with PHP\'s PCRE library or an HTML filter corrupting the %1$s meta tags.', 'nextgen-facebook' ), $short ) );
						return false;
					}
				}
			}
