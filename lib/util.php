	...
	class NgfbUtil extends SucomUtil {
		...
		public function clear_all_cache() {
			...
			if ( function_exists( 'w3tc_pgcache_flush' ) ) {	// w3 total cache
				w3tc_pgcache_flush();
				$this->p->notice->inf( __( 'W3 Total Cache has been cleared as well.', 'nextgen-facebook' ), true );
			}
			if ( function_exists( 'wp_cache_clear_cache' ) ) {	// wp super cache
				wp_cache_clear_cache();
				$this->p->notice->inf( __( 'WP Super Cache has been cleared as well.', 'nextgen-facebook' ), true );
			}
			if ( isset( $GLOBALS['zencache'] ) ) {			// zencache
				$GLOBALS['zencache']->wipe_cache();
				$this->p->notice->inf( __( 'ZenCache has been cleared as well.', 'nextgen-facebook' ), true );
			}
		}
		...
		public function get_topics() {
			...
			if ( ( $topics = file( NGFB_TOPICS_LIST, 
				...
				$this->p->notice->err( sprintf( 'Error reading the %s topic list file.', NGFB_TOPICS_LIST ) );
				return $topics;
			}
