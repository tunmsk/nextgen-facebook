	...
	class NgfbUtil extends SucomUtil {
		...
		public function get_head_meta( $request, $query = '/html/head/meta', $remove_self = false ) {
			...
			if ( $remove_self && strpos( $html, $lca.' meta tags begin' ) !== false ) {	// quick check
				...
				if ( ! $count ) {
					if ( is_admin() ) {
						$short = $this->p->cf['plugin'][$lca]['short'];
						$this->p->notice->err( sprintf( __( 'The PHP preg_replace() function failed to remove the %s meta tag block - this could be an indication of an issue with the PHP PCRE library.', 'nextgen-facebook' ), $short ) );
						return false;
					}
				}
			}
				...
				if ( ! $disabled && ! empty( $media_url ) && strpos( $media_url, '://' ) !== false ) {
					...
					if ( $image_info === false ) {
						...
						if ( is_array( $image_info ) ) {
							...
							*/
							...
							$image_info = array( NGFB_UNDEF_INT, NGFB_UNDEF_INT, '', '' );
						}
					}
					...
					foreach ( array( 'width', 'height' ) as $attr )
						if ( isset( $opts[$prefix.':'.$attr] ) )
							$opts[$prefix.':'.$attr] = NGFB_UNDEF_INT;
				}
			}
		...
		public function rename_opts_by_ext( &$opts, $options_keys ) {
			...
			foreach ( $this->p->cf['plugin'] as $ext => $info ) {
				...
				$opt_version = empty( $opts['plugin_'.$ext.'_opt_version'] ) ?
					0 : $opts['plugin_'.$ext.'_opt_version'];

				foreach ( $options_keys[$ext] as $max_version => $keys ) {
					if ( is_numeric( $max_version ) && $opt_version > 0 &&
						$opt_version <= $max_version && is_array( $keys ) ) {

						SucomUtil::rename_keys( $opts, $keys, true );	// $modifiers = true
