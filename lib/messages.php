			 ...
			 */
				...
				switch ( $idx ) {
					...
					case 'info-image-dimensions':
						$text = '<blockquote class="top-info"><p>'.sprintf( __( '%s allows you to control image dimensions based on their intended use (Facebook / Open Graph, Twitter Cards, Pinterest Rich Pins, etc.).', 'nextgen-facebook' ), $info['short'] ).' '.$fb_recommends.'</p></blockquote>';
						break;
				...
				switch ( $idx ) {
					case 'notice-image-rejected':
						...
						if ( current_user_can( 'manage_options' ) ) {
							...
							$text .= '<li>'.sprintf( __( 'You can also adjust the <b>%2$s</b> option in the <a href="%1$s">Social Image Dimensions</a> settings.',
								'nextgen-facebook' ), $this->p->util->get_admin_url( 'image-dimensions' ), $info['size_label'] ).'</li>';
					...
					case 'notice-um-activate-extension':
						...
						if ( $idx === 'notice-um-extension-required' ) {
							$text .= sprintf( __( 'This Free plugin is required to update and enable the %s plugin and its Pro extensions.', 'nextgen-facebook' ), $info['name_pro'] ).'</p><ol><li><b>'.sprintf( __( 'Download the Free <a href="%1$s">%2$s plugin archive</a> (ZIP).', 'nextgen-facebook' ), $um_latest, $um_name ).'</b></li><li><b>'.sprintf( __( 'Then <a href="%s">upload and activate the plugin</a> on the WordPress plugin upload page.', 'nextgen-facebook' ), $upload_url ).'</b></li></ol>';
						} else $text .= '</p>';
