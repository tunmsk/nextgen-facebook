				...
				if ( strpos( $idx, 'info-meta-' ) === 0 ) {
					switch ( $idx ) {
						...
						case 'info-meta-validate-amp':
							$text = '<p>'.__( 'Validate the HTML syntax and HTML AMP conformance of your meta tags and the AMP markup of your templates.', 'nextgen-facebook' ).'</p>'.( $this->p->is_avail['amp_endpoint'] ? '' : '<p><i>'.sprintf( __( 'The <a href="%s">AMP plugin by Automattic</a> is required to validate AMP formatted webpages.', 'nextgen-facebook' ), 'https://wordpress.org/plugins/amp/' ).'</i></p>' );
						 	break;
					...
					switch ( $idx ) {
						...
						case 'info-plugin-tid-network':	// displayed on the Network Pro Licenses settings page
							...
							$text .= sprintf( __( 'You may enter the Authentication IDs on this page <em>to define a value for all sites within the network</em> &mdash; or enter the Authentication IDs individually on each site\'s %1$s settings page.', 'nextgen-facebook' ), $ext_menu_html ).' ';
							
							$text.= __( 'If you enter Authentication IDs on this page, <em>make sure you have purchased enough licenses for all sites within the network</em> (for example, if you have 10 sites, you will need at least 10 licenses).', 'nextgen-facebook' ).' ';
	
							$text .= '</p><p>'.sprintf( __( 'Please note that <strong>the default site / blog must be licensed</strong>, and the %1$s extension active, in order to install Pro version updates from the network admin interface.', 'nextgen-facebook' ), $um_info['name'] ).'</p></blockquote>';
							break;
				...
				switch ( $idx ) {
					case 'column-purchase-pro':
						...
						if ( $this->p->is_avail['aop'] ) {
							$text .= sprintf( __( '<strong>Purchase %s quickly and easily with PayPal</strong> &mdash; allows you to license the Pro version within seconds of your purchase.', 'nextgen-facebook' ), $info['short_pro'] );
						} else {
							$text .= sprintf( __( '<strong>Purchase %s quickly and easily with PayPal</strong> &mdash; allows you to update the Free plugin within seconds of your purchase.', 'nextgen-facebook' ), $info['short_pro'] );
						}
						$text .= '</p><p>'.__( '<strong>Pro licenses never expire</strong> &mdash; there are no yearly fees for support and updates!', 'nextgen-facebook' ).'</p>';
						break;

					case 'column-help-support':
						$text = '<p>'.sprintf( __( '<strong>The development of %1$s is driven by user requests</strong> &mdash; we welcome your suggestions to improve the plugin and its extensions.', 'nextgen-facebook' ), $info['short'] ).'</p>';
						break;

					case 'column-rate-review':
						$text = '<p>'.sprintf( __( '<strong>A terrific rating is a great way to encourage us!</strong> &mdash; and it only takes a minute or less.', 'nextgen-facebook' ), $info['short'] ).' ;-)</p>';
						break;
