				 ...
				 */
					...
					switch ( $idx ) {
						...
						case 'tooltip-plugin_og_img_col':
							$col_name = sprintf( _x( '%s Img', 'column title', 'nextgen-facebook' ), $this->p->cf['menu_label'] );
							// no break
						case 'tooltip-plugin_og_desc_col':
							if ( empty( $col_name ) )
								$col_name = sprintf( _x( '%s Desc', 'column title', 'nextgen-facebook' ), $this->p->cf['menu_label'] );
						...
						case 'tooltip-plugin_column_cache_exp':
							$cache_exp = NgfbConfig::$cf['opt']['defaults']['plugin_column_cache_exp'];	// use original un-filtered value
							$cache_diff = $cache_exp ? human_time_diff( 0, $cache_exp ) : _x( 'disabled', 'option comment', 'nextgen-facebook' );
