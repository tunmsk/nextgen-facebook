				...
				if ( strpos( $idx, 'tooltip-side-' ) === 0 ) {
					switch ( $idx ) {
						...
						case 'tooltip-side-wp-image-upscaler':
							$text = $atts['short_pro'].' includes a special filter for WordPress than allows it to upscale ALL Media Library images <em>when necessary</em>. You can enable the WP Image Upscaler from the '.$this->p->util->get_admin_url( 'advanced#sucom-tabset_plugin-tab_integration', 'Advanced Settings' ).' page.';
							break;
