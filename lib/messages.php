			 ...
			 */
				...
				switch ( $idx ) {
					...
					case 'info-plugin-tid':
						...
						$text = '<blockquote class="top-info"><p>'.sprintf( __( 'After purchasing license(s) for %1$s or its Pro extensions, you\'ll receive an email with a unique Authentication ID and installation instructions.', 'nextgen-facebook' ), $info['short_pro'] ).' '.__( 'Enter the Authentication IDs in this settings page to upgrade the Free version and access future Pro version updates.', 'nextgen-facebook' ).' '.sprintf( __( 'The %1$s extension must be active in order to check for Pro version updates, and %2$s is required to use any of its Pro extensions.', 'nextgen-facebook' ), $um_info['name'], $info['short_pro'] ).'</blockquote>';
						break;
					case 'info-plugin-tid-network':
						$um_info = $this->p->cf['plugin']['ngfbum'];
						$text = '<blockquote class="top-info"><p>'.sprintf( __( 'After purchasing license(s) for %1$s or its Pro extensions, you\'ll receive an email with a unique Authentication ID and installation instructions.', 'nextgen-facebook' ), $info['short_pro'] ).' '.__( 'You may enter the Authentication IDs on this page <em>to define a value for all sites within the network</em> &mdash; or enter the Authentication IDs individually on each site\'s Pro Licenses settings page.', 'nextgen-facebook' ).' '.__( 'If you enter Authentication IDs here, <em>make sure you have purchased enough licenses to license all sites within the network</em> (for example, if you have 10 sites, you will need at least 10 licenses).', 'nextgen-facebook' ).' '.__( 'To license one or more sites individually, enter the Authentication ID in each site\'s Pro Licenses settings page.', 'nextgen-facebook' ).'</p><p>'.sprintf( __( 'Note that <strong>the default site / blog must be licensed</strong> and the %1$s extension active in order to install Pro version updates from the network admin interface.', 'nextgen-facebook' ), $um_info['name'] ).'</p></blockquote>';
						break;
			 ...
			 */
				...
				switch ( $idx ) {
					...
					case 'notice-um-activate-extension':
						...
						$text .= '<p>'.sprintf( __( 'Once the %s extension has been activated, one or more Pro version updates may be available for your licensed plugin(s).', 'nextgen-facebook' ), $um_info['name'] ).' '.sprintf( __( 'Read more <a href="%1$s" target="_blank">about the %2$s extension plugin</a>.', 'nextgen-facebook' ), $um_info['url']['home'], $um_info['name'] ).'</p>';
						break;
					case 'notice-um-version-required':
						$um_info = $this->p->cf['plugin']['ngfbum'];
						$um_version = isset( $um_info['version'] ) ? $um_info['version'] : 'unknown';
						$text = sprintf( __( '%1$s version %2$s requires the use of %3$s version %4$s or newer (version %5$s is currently installed).', 'nextgen-facebook' ), $info['name_pro'], $this->p->cf['plugin']['ngfb']['version'], $um_info['short'], $info['min_version'], $um_version ).' '.sprintf( __( 'Use the <em>%1$s</em> button from any %2$s settings page to retrieve the latest update information, or <a href="%3$s" target="_blank">download the latest %4$s extension version</a> and install the ZIP file manually.', 'nextgen-facebook' ), _x( 'Check for Pro Update(s)', 'submit button', 'nextgen-facebook' ), $this->p->cf['menu']['label'], $um_info['url']['home'], $um_info['short'] );
						break;
