				 ...
				 */
					...
					switch ( $idx ) {
						...
						case 'tooltip-og_author_fallback':
							$text = sprintf( __( 'If the Author Profile URL (and the Author Link URL in the Google Settings below) is not a valid URL, then %1$s can fallback to using the author index / archive page on this website (\'%2$s\' for example).', 'nextgen-facebook' ), $atts['short'], trailingslashit( site_url() ).'author/username' ).' '.__( 'Uncheck this option to disable the fallback feature (default is unchecked).', 'nextgen-facebook' );
							break;
				 ...
				 */
					...
					switch ( $idx ) {
						...
						case 'tooltip-plugin_google_api_key':
							$text = sprintf( __( 'The Google BrowserKey for this website (project). If you don\'t already have one, visit <a href="%s" target="_blank">Google\'s Cloud Console</a> and create a new project for your website (use the \'Select a project\' drop-down).', 'nextgen-facebook' ), 'https://console.developers.google.com/start' );
							break;
				 ...
				 */
					...
					switch ( $idx ) {
						...
						case 'tooltip-rp_author_name':
							$text = __( 'Pinterest ignores Facebook-style Author Profile URLs in the \'article:author\' Open Graph meta tags.', 'nextgen-facebook' ).' '.__( 'An additional \'article:author\' meta tag can be included when the Pinterest crawler is detected.', 'nextgen-facebook' ).' '.__( 'Select an Author Name Format to use, or \'[none]\' to disable this feature (the recommended value is \'Display Name\').', 'nextgen-facebook' );
							break;
						case 'tooltip-rp_dom_verify':
							$text = sprintf( __( 'To <a href="%s" target="_blank">verify your website</a> with Pinterest, edit your business account profile on Pinterest and click the "Verify Website" button.', 'nextgen-facebook' ), 'ttps://help.pinterest.com/en/articles/verify-your-website#meta_tag' ).' '.__( 'Enter the supplied \'p:domain_verify\' meta tag <em>content</em> value here.', 'nextgen-facebook' );
							break;
						default:
			 ...
			 */
				...
				switch ( $idx ) {
					...
					case 'pro-um-activate-extension':
						...
						$text = '<p>'.sprintf( __( 'One or more Authentication IDs have been entered on the <a href="%1$s">Pro Licenses settings page</a>, but the <strong>%2$s</strong> plugin is not active.', 'nextgen-facebook' ), $this->p->util->get_admin_url( 'licenses' ), $um_name ).' ';

						if ( $idx === 'pro-um-extension-required' ) {
							$text .= sprintf( __( 'This <strong>Free extension</strong> is required to update and enable the %s plugin and its extensions.', 'nextgen-facebook' ), $atts['name_pro'] ).'</p>';
							$text .= '<ol><li><strong>'.sprintf( __( 'Download the Free <a href="%1$s">%2$s plugin archive</a> (ZIP).', 'nextgen-facebook' ), $um_latest, $um_name ).'</strong></li>';
							$text .= '<li><strong>'.sprintf( __( 'Then <a href="%s">upload and activate the plugin</a> on the WordPress plugin upload page.', 'nextgen-facebook' ), $upload_url ).'</strong></li></ol>';
						} else $text .= '</p>';

						$text .= '<p>'.sprintf( __( 'Once the %s extension has been activated, one or more Pro version updates may be available for your licensed plugin(s).', 'nextgen-facebook' ), $um_name ).'</p>';
						$text .= '<p>'.sprintf( __( 'Read more <a href="%1$s" target="_blank">about the %2$s extension plugin</a>.', 'nextgen-facebook' ), $um_dl, $um_name ).'</p>';
						break;
					default:
