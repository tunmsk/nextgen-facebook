				 ...
				 */
					...
					switch ( $idx ) {
						...
						case 'tooltip-plugin_preserve':	// Preserve Settings on Uninstall
							$text = sprintf( __( 'Check this option if you would like to preserve all %s settings when you <em>uninstall</em> the plugin (default is unchecked).', 'nextgen-facebook' ), $info['short'] );
							break;
						case 'tooltip-plugin_debug':	// Add Hidden Debug Messages
							$text = __( 'Add debugging messages to back-end and front-end webpages as hidden HTML comments (default is unchecked).', 'nextgen-facebook' );
							break;
						...
						case 'tooltip-plugin_show_opts':	// Options to Show by Default
							$text = sprintf( __( 'Select the default set of options to display in the %s settings pages. The basic view shows only the most commonly used options.', 'nextgen-facebook' ), $info['short'] );
							break;
						/*
			 ...
			 */
				...
				switch ( $idx ) {
					...
					case 'notice-recommend-version':
						$text = sprintf( __( 'You are using %1$s version %2$s &mdash; <a href="%3$s">this version is outdated, unsupported, and insecure</a> (and lacks important features). Please update to the latest %1$s stable production release (or at least version %3$s).', 'nextgen-facebook' ), $info['app_label'], $info['cur_version'], $info['rec_version'], $info['version_url'] );
						break;
					default:
