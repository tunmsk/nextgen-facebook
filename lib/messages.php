			 ...
			 */
				...
				switch ( $idx ) {
					...
					case 'notice-um-activate-extension':
						...
						$text = '<p><b>'.sprintf( __( 'At least one Authentication ID has been entered on the <a href="%1$s">Pro Licenses settings page</a>, but the %2$s plugin is not active.', 'nextgen-facebook' ), $this->p->util->get_admin_url( 'licenses' ), $um_info['name'] ).'</b> ';
					...
					case 'notice-um-version-required':
						$um_info = $this->p->cf['plugin']['ngfbum'];
						$text = sprintf( __( '%1$s version %2$s requires the use of %3$s version %4$s or newer (version %5$s is currently installed).', 'nextgen-facebook' ), $info['name_pro'], $this->p->cf['plugin']['ngfb']['version'], $um_info['short'], $info['um_min_version'], $um_info['version'] ).' '.sprintf( __( 'Use the <em>%1$s</em> button from any %2$s settings page to retrieve the latest update information, or <a href="%3$s" target="_blank">download the latest %4$s extension version</a> and install the ZIP file manually.', 'nextgen-facebook' ), _x( 'Check for Pro Update(s)', 'submit button', 'nextgen-facebook' ), $this->p->cf['menu'], $um_info['url']['download'], $um_info['short'] );
						break;
					default:
