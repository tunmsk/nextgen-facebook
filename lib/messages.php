				...
				if ( strpos( $idx, 'tooltip-side-' ) === 0 ) {
					switch ( $idx ) {
						...
						case 'tooltip-side-google-person-markup':
							$text = __( 'Add Person social profiles markup to the home page header for Google Search.', 'nextgen-facebook' );
							break;
						case 'tooltip-side-google-puborg-markup':
							$text = __( 'Add Organization social profiles markup to the home page header for Google Search.', 'nextgen-facebook' );
							break;
						case 'tooltip-side-google-website-markup':
							$text = __( 'Add Website markup to the home page header Google Search. The Website markup includes the home page URL, name, alternate name, and search query URL.', 'nextgen-facebook' );
							break;
				 ...
				 */
					...
					switch ( $idx ) {
						...
						case 'tooltip-meta-og_vid_desc':
							$text = sprintf( __( 'The %1$s video API modules retrieve the video name / title and description <em>when available</em>.', 'nextgen-facebook' ), $atts['short_pro'] ).' '.__( 'The video name / title and description text is used for Schema JSON-LD markup (extension plugin required), which can be read by both Google and Pinterest.', 'nextgen-facebook' );
							break;
				 ...
				 */
					...
					switch ( $idx ) {
						...
						case 'tooltip-og_site_name':
							$text = sprintf( __( 'The WordPress Site Name is used for the Facebook / Open Graph and Pinterest Rich Pin site name (og:site_name) meta tag. You may override <a href="%s">the default WordPress Site Title value</a>.', 'nextgen-facebook' ), get_admin_url( null, 'options-general.php' ) );
							break;
				 ...
				 */
					...
					switch ( $idx ) {
						...
						case 'tooltip-google_schema_author_name':
							$text = sprintf( __( 'Select an \'%1$s\' for the author / Person markup, or \'[None]\' to exclude it from the author / Person markup (the recommended value is \'Display Name\').', 'nextgen-facebook' ), _x( 'Author Name Format', 'option label', 'nextgen-facebook' ) );
							break;
			 ...
			 */
				...
				switch ( $idx ) {
					case 'pro-feature-msg':
						...
						break;
					case 'pro-option-msg':
						$text = '<p class="pro-option-msg"><a href="'.$url['purchase'].'" target="_blank">'.sprintf( _x( '%s required to use this option', 'option comment', 'nextgen-facebook' ), $atts['short_pro'] ).'</a></p>';
						break;
					default:
			 ...
			 */
				...
				switch ( $idx ) {
					case 'notice-image-rejected':
						...
						$text = __( 'Please select or upload a larger / different featured image. The Priority Media tab bellow can also be used to select an image specifically for social sharing purposes.', 'nextgen-facebook' );
						if ( current_user_can( 'manage_options' ) ) {
							...
							$text .= '<li>'.sprintf( __( 'You can also adjust the <b>%2$s</b> option in the <a href="%1$s">Social Image Dimensions</a> settings.',
								'nextgen-facebook' ), $this->p->util->get_admin_url( 'image-dimensions' ), $atts['size_label'] ).'</li>';
					...
					case 'notice-missing-og-image':
						$text = __( 'An Open Graph image meta tag could not be created from this webpage content. Facebook and other social websites <em>require</em> at least one Open Graph image meta tag to render shared content correctly.', 'nextgen-facebook' ).' '.__( 'You may select an optional customized image &ndash; for Facebook and other social websites &ndash; in the Social Settings metabox under the Priority Media tab.', 'nextgen-facebook' );
						break;
