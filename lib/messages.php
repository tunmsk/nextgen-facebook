				...
				if ( strpos( $idx, 'tooltip-side-' ) === 0 ) {
					switch ( $idx ) {
						...
						case 'tooltip-side-author-json-ld':
							$text = __( 'Include author (Person) social profiles markup to webpage headers in schema.org JSON-LD format for Google Search.', 'nextgen-facebook' );
							break;
						case 'tooltip-side-debug-messages':
							$text = sprintf( __( 'The debug library is loaded when the <em>Add Hidden Debug Messages</em> option is checked, or one of the debugging <a href="%s" target="_blank">constants</a> is defined.', 'nextgen-facebook' ), 'http://surniaulula.com/codex/plugins/nextgen-facebook/notes/constants/' );
							break;
						case 'tooltip-side-non-persistant-cache':
							$text = sprintf( __( 'The plugin saves filtered / rendered content to a non-persistant cache (aka <a href="%1$s" target="_blank">WP Object Cache</a>) for re-use within the same page load. You can disable the use of non-persistant cache (not recommended) using one of the available <a href="%2$s" target="_blank">constants</a>.', 'nextgen-facebook' ), 'https://codex.wordpress.org/Class_Reference/WP_Object_Cache', 'http://surniaulula.com/codex/plugins/nextgen-facebook/notes/constants/' );
							break;
						case 'tooltip-side-open-graph-rich-pin':
							$text = __( 'Facebook / Open Graph and Pinterest Rich Pin meta tags are added to the head section of all webpages. You must have a supported eCommerce plugin installed to add <em>Product</em> Rich Pins, including product prices and attributes.', 'nextgen-facebook' );
							break;
						case 'tooltip-side-publisher-json-ld':
							$text = __( 'Include publisher (Organization) social profiles markup to webpage headers in schema.org JSON-LD format for Google Search.', 'nextgen-facebook' );
							break;
						case 'tooltip-side-transient-cache':
							$text = sprintf( __( 'The plugin saves Facebook / Open Graph, Pinterest Rich Pin, Twitter Card meta tags, and JSON-LD markup to a persistant (aka <a href="%1$s" target="_blank">Transient</a>) cache for %2$d seconds. You can adjust the Transient cache expiration value on the <a href="%3$s">%4$s</a> settings page, or disable it completely by using one of the available <a href="%5$s" target="_blank">constants</a>.', 'nextgen-facebook' ), 'https://codex.wordpress.org/Transients_API', $this->p->options['plugin_object_cache_exp'], $this->p->util->get_admin_url( 'advanced' ), _x( 'Advanced', 'lib file description', 'nextgen-facebook' ), 'http://surniaulula.com/codex/plugins/nextgen-facebook/notes/constants/' );
							break;
						case 'tooltip-side-twitter-cards':
							$text = __( 'Twitter Cards extend the standard Facebook / Open Graph and Pinterest Rich Pin meta tags with additional information about your content. Twitter Cards are displayed more prominently on Twitter, allowing you to highlight your content more effectively.', 'nextgen-facebook' );
							break;
						/*
						 * Pro version
						 */
				 ...
				 */
					...
					switch ( $idx ) {
						...
						case 'tooltip-og_author_field':
							$text = __( 'Select which contact field to use from the author\'s profile page for the Facebook / Open Graph and Pinterest Rich Pin \'article:author\' meta tag(s). The preferred setting is the Facebook URL field (default value).', 'nextgen-facebook' );
							break;
						case 'tooltip-og_author_fallback':
							$text = sprintf( __( 'If the \'%1$s\' (and the \'%2$s\' in the Google settings below) is not a valid URL, then %3$s can fallback to using the author index / archive page on this website (for example, \'%4$s\').', 'nextgen-facebook' ), _x( 'Author Profile URL Field', 'option label', 'nextgen-facebook' ), _x( 'Author Link URL Field', 'option label', 'nextgen-facebook' ), $atts['short'], trailingslashit( site_url() ).'author/username' ).' '.__( 'Uncheck this option to disable the fallback feature (default is unchecked).', 'nextgen-facebook' );
							break;
				 ...
				 */
					...
					switch ( $idx ) {
						...
						case 'tooltip-plugin_head_attr_filter':
							$text = $atts['short'].' hooks the \'language_attributes\' filter by default to modify the &amp;lt;html&amp;gt; attributes for the Schema itemscope. The &amp;lt;head&amp;gt; element is actually the preferred location for this markup but WordPress does not offer a standard filter for &amp;lt;head&amp;gt; attributes. If your theme offers a filter for &amp;lt;head&amp;gt; attributes, enter its name here. Alternatively, you can add a function call instead. Example header.php template code: <pre><code>&amp;lt;head &amp;lt;?php '.$lca.'_schema_attributes(); ?&amp;gt;&amp;gt;</code></pre> Leave the option value empty if you add this function to your template.';
							break;
						/*
						...
						case 'tooltip-plugin_shortener':
							$text = sprintf( __( 'A preferred URL shortening service for %s plugin filters and/or extensions that may need to shorten URLs &mdash; don\'t forget to define the Service API Keys for the URL shortening service of your choice.', 'nextgen-facebook' ), $atts['short'] );
							break;
						...
						case 'tooltip-plugin_bitly_login':
							$text = sprintf( __( 'The username for your Bit.ly API key (see <a href="%s" target="_blank">Your Bit.ly API Key</a> for details).', 'nextgen-facebook' ), 'https://bitly.com/a/your_api_key' );
							break;
						case 'tooltip-plugin_bitly_api_key':
							$text = sprintf( __( 'To use Bit.ly as your preferred shortening service, you must provide the Bit.ly API key for this website (see <a href="%s" target="_blank">Your Bit.ly API Key</a> for details).', 'nextgen-facebook' ), 'https://bitly.com/a/your_api_key' );
							break;
				 ...
				 */
					...
					switch ( $idx ) {
						...
						case 'tooltip-google_author_name':
							$text = sprintf( __( 'Select an \'%1$s\' to use for the \'%2$s\' meta tag, or \'[none]\' to disable this feature (the recommended value is \'Display Name\').', 'nextgen-facebook' ), _x( 'Author Name Format', 'option label', 'nextgen-facebook' ), 'author' ).' Facebook uses the "author" meta tag value to credit the author on timeline shares, but the <strong>Facebook Debugger will show a warning</strong> &mdash; thus it is disabled by default. Now that you know about the false warning from the Facebook Debugger, you should set this option to \'Display Name\'. ;-)';
							break;
				 ...
				 */
					...
					switch ( $idx ) {
						...
						case 'tooltip-rp_author_name':
							$text = __( 'Pinterest ignores Facebook-style Author Profile URLs in the \'article:author\' Open Graph meta tags.', 'nextgen-facebook' ).' '.__( 'An additional \'article:author\' meta tag can be included when the Pinterest crawler is detected.', 'nextgen-facebook' ).' '.sprintf( __( 'Select an \'%1$s\' to use for the \'%2$s\' meta tag, or \'[none]\' to disable this feature (the recommended value is \'Display Name\').', 'nextgen-facebook' ), _x( 'Author Name Format', 'option label', 'nextgen-facebook' ), 'article:author' );
							break;
			 ...
			 */
				...
				switch ( $idx ) {
					case 'info-plugin-tid':
						...
						$text = '<blockquote class="top-info"><p>'.__( 'After purchasing Pro version license(s), an email is sent to you with a unique Authentication ID and installation / activation instructions.', 'nextgen-facebook' ).' '.__( 'Enter the unique Authentication ID on this settings page to check for Pro version updates immediately, and every 24 hours thereafter.', 'nextgen-facebook' ).'</p><p><strong>'.sprintf( __( 'The %s extension must be active in order to check for Pro version updates.', 'nextgen-facebook' ), $um_name ).'</strong> '.sprintf( __( 'If you accidentally de-activate the %1$s extension, update information will be provided by the WordPress.org plugin repository, and any update notices will be for the Free versions &mdash; always update the Pro version when the %2$s extension is active.', 'nextgen-facebook' ), $um_short, $um_short ).' '.__( 'If you accidentally re-install the Free version from WordPress.org &ndash; don\'t worry &ndash; your Authentication ID will always allow you update back to the Pro version.', 'nextgen-facebook' ).' ;-)</p></blockquote>';
						break;
					case 'info-plugin-tid-network':
						...
						$text = '<blockquote class="top-info"><p>'.__( 'After purchasing Pro version license(s), an email is sent to you with a unique Authentication ID and installation / activation instructions.', 'nextgen-facebook' ).' '.__( 'Enter the unique Authentication ID on this page to define a default / forced value for <em>all</em> sites within the network, or enter the Authentication ID(s) individually on each site\'s Pro Licenses settings page.', 'nextgen-facebook' ).' <strong>'.sprintf( __( 'Please note that the <em>default</em> site / blog must be licensed and the %1$s extension must be active in order to install %2$s Pro updates from the Network admin interface.', 'nextgen-facebook' ), $um_name, $atts['short'] ).'</strong></p></blockquote>';
						break;
					case 'info-review':
						$text = '<blockquote class="top-info"><p>'.sprintf( __( 'If you appreciate the features, quality, and/or support of this plugin, please <a href="%1$s" target="_blank">take a moment to rate the %2$s plugin on WordPress.org</a>.', 'nextgen-facebook' ), $url['review'], $atts['short'] ).' '.sprintf( __( 'Your rating will help other WordPress users find higher quality and better supported plugins &mdash; and <strong>encourage us to keep improving %s</strong> as well!', 'nextgen-facebook' ), $atts['short'] ).' ;-)</p></blockquote>';
						break;
					case 'info-pub-pinterest':
						$text = '<blockquote class="top-info"><p>'.__( 'Pinterest uses the Open Graph standard meta tags for their Rich Pins.', 'nextgen-facebook' ).' '.__( 'These options allow you to manage and/or override some Pinterest-specific Open Graph settings.', 'nextgen-facebook' ).' '.__( 'Please note that if you use a caching plugin, or front-end caching service, it should detect the Pinterest crawler user-agent and bypass its cache (for example, look for a <em>User-Agent Exclusion Pattern</em> option and add "Pinterest/" to that list).', 'nextgen-facebook' ).' '.sprintf( __( 'This will allow %s to provide different / customized meta tags specifically for the Pinterest crawler.', 'nextgen-facebook' ), $atts['short'] ).'</p></blockquote>';
						break;
					case 'info-pub-twitter':
						$text = '<blockquote class="top-info"><p><strong>'.__( 'The Photo, Gallery, and Product Cards were deprecated by Twitter on July 3rd, 2015.', 'nextgen-facebook' ).'</strong> '.sprintf( __( '%1s continues to support all <a href="%2s">current Twitter Card formats</a>, including the Summary, Summary with Large Image, App (extension plugin required), and Player Cards.', 'nextgen-facebook' ), $atts['short'], 'https://dev.twitter.com/cards/types' ).'</p></blockquote>';
						break;
					case 'info-cm':
						$text = '<blockquote class="top-info"><p>'.sprintf( __( 'The following options allow you to customize the contact field names and labels shown on <a href="%s">the user profile page</a>.', 'nextgen-facebook' ), get_admin_url( null, 'profile.php' ) ).' '.sprintf( __( '%s uses the Facebook, Google+, and Twitter contact fields for Facebook / Open Graph, Schema, and Twitter Card meta tags.', 'nextgen-facebook' ), $atts['short'] ).' <strong>'.sprintf( __( 'You should not modify the <em>%s</em> unless you have a <em>very</em> good reason to do so.', 'nextgen-facebook' ), _x( 'Contact Field Name', 'column title', 'nextgen-facebook' ) ).'</strong> '.sprintf( __( 'The <em>%s</em> on the other hand is for display purposes only and it can be changed as you wish.', 'nextgen-facebook' ), _x( 'Profile Contact Label', 'column title', 'nextgen-facebook' ) ).' '.sprintf( __( 'Enabled contact methods are shown on user profile pages automatically, but your theme is responsible for displaying them in other locations throughout your website (see the WordPress <a href="%s" target="_blank">get_the_author_meta()</a> documentation for examples).', 'nextgen-facebook' ), 'https://codex.wordpress.org/Function_Reference/get_the_author_meta' ).'</p><p><center><strong>'.__( 'DO NOT ENTER YOUR CONTACT INFORMATION HERE &ndash; THESE ARE CONTACT FIELD LABELS ONLY.', 'nextgen-facebook' ).'</strong><br/>'.sprintf( __( 'Enter your personal contact information on <a href="%1$s">the user profile page</a>.', 'nextgen-facebook' ), get_admin_url( null, 'profile.php' ) ).'</center></p></blockquote>';
						break;
					case 'info-taglist':
						$text = '<blockquote class="top-info"><p>'.sprintf( __( '%s adds the following Google / SEO, Facebook, Open Graph, Rich Pin, Schema, and Twitter Card HTML tags to the <code>&lt;head&gt;</code> section of your webpages.', 'nextgen-facebook' ), $atts['short'] ).' '.__( 'If your theme or another plugin already creates one or more of these HTML tags, you can uncheck them here to prevent duplicates from being added.', 'nextgen-facebook' ).' '.__( 'As an example, the "meta name description" HTML tag is automatically unchecked if a <em>known</em> SEO plugin is detected.', 'nextgen-facebook' ).' '.__( 'The "meta name canonical" HTML tag is unchecked by default since themes often include this meta tag in their <code>header.php</code> template file.', 'nextgen-facebook' ).'</p></blockquote>';
						break;
					...
					case 'info-sharing-include':
						$text = '<blockquote class="top-info"><p>'.__( 'The buttons enabled bellow (along with those in the widget) can be included or excluded as a group based on the webpage content type.', 'nextgen-facebook' ).' '.__( 'This does <em>not</em> apply to the shortcode and/or function buttons, which are managed with their own parameter options.', 'nextgen-facebook' ).'</p></blockquote>';
						break;
					default:
				...
				switch ( $idx ) {
					case 'pro-feature-msg':
						if ( $this->p->check->aop( $lca, false ) )
							$text = '<p class="pro-feature-msg"><a href="'.$url['purchase'].'" target="_blank">'.sprintf( __( 'Purchase %s licence(s) to install / enable Pro modules and modify the following options.', 'nextgen-facebook' ), $atts['short_pro'] ).'</a></p>';
						else $text = '<p class="pro-feature-msg"><a href="'.$url['purchase'].'" target="_blank">'.sprintf( __( 'Purchase the %s plugin to install / enable Pro modules and modify the following options.', 'nextgen-facebook' ), $atts['short_pro'] ).'</a></p>';
						break;
					case 'pro-option-msg':
						$text = '<p class="pro-option-msg"><a href="'.$url['purchase'].'" target="_blank">'.sprintf( __( '%s required to use this option', 'option comment', 'nextgen-facebook' ), $atts['short_pro'] ).'</a></p>';
						break;
					default:
			 ...
			 */
				...
				switch ( $idx ) {
					...
					case 'notice-content-filters-disabled':
						$text = '<b>'.sprintf( __( 'The <a href="%1$s">%2$s</a> advanced option is currently disabled.', 'nextgen-facebook' ), $this->p->util->get_admin_url( 'advanced#sucom-tabset_plugin-tab_content' ), _x( 'Apply WordPress Content Filters', 'option label', 'nextgen-facebook' ) ).'</b> '.sprintf( __( 'The use of WordPress content filters allows %s to fully render your content text for meta tag descriptions, and detect additional images / embedded videos provided by shortcodes.', 'nextgen-facebook' ), $atts['short'] ).'<br/><br/><b>'.__( 'Some theme / plugins have badly coded content filters, so this option is disabled by default.', 'nextgen-facebook' ).'</b> '.sprintf( __( '<a href="%s">If you use any shortcodes in your content text, this option should be enabled</a> (Pro version required) &mdash; if you experience display issues after enabling this option, determine which theme / plugin content filter is at fault, and report the problem to its author(s).', 'nextgen-facebook' ), $this->p->util->get_admin_url( 'advanced#sucom-tabset_plugin-tab_content' ) ).'<br/><br/>'.__( 'This options check and notice display are only performed once when updating the plugin.', 'nextgen-facebook' );
						break;
					case 'notice-head-attr-filter-default':
						$text = '<b>'.sprintf( __( 'The <a href="%1$s">%2$s</a> advanced option is currently set to its default value of "language_attributes".', 'nextgen-facebook' ), $this->p->util->get_admin_url( 'advanced#sucom-tabset_plugin-tab_integration'), _x( '&lt;head&gt; Attributes Filter Hook', 'option label', 'nextgen-facebook' ) ).'</b> '.sprintf( __( 'If your theme does not offer a filter hook for the &lt;head&gt; element attributes, consider adding a call to the %1$s function in your theme\'s header.php template instead. Example: %2$s', 'nextgen-facebook' ), '<code>'.$lca.'_schema_attributes()</code>', '<pre><code>&lt;head &lt;?php '.$lca.'_schema_attributes(); ?&gt;&gt;</code></pre>' ).__( 'Leave the option value empty if you add this function to your template.', 'nextgen-facebook' );
						break;
				...
				switch ( $idx ) {
					case 'side-purchase':
						...
						if ( $this->p->is_avail['aop'] )
							$text .= sprintf( __( '%s can be purchased quickly and easily via Paypal &ndash; allowing you to license and enable Pro version features within seconds of your purchase.', 'nextgen-facebook' ), $atts['short_pro'] );
						else $text .= sprintf( __( '%s can be purchased quickly and easily via Paypal &ndash; allowing you to update the plugin within seconds of your purchase.', 'nextgen-facebook' ), $atts['short_pro'] );
