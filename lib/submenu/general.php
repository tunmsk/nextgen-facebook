...
if ( ! class_exists( 'NgfbSubmenuGeneral' ) && class_exists( 'NgfbAdmin' ) ) {
	...
	class NgfbSubmenuGeneral extends NgfbAdmin {
		...
		protected function get_rows( $metabox, $key ) {
			$rows = array();
			$this->form->user_ids = $this->p->m['util']['user']->get_display_names();
			$this->form->author_contact_fields = $this->p->m['util']['user']->get_contact_fields();
			...
			switch ( $metabox.'-'.$key ) {
				...
				case 'pub-google':
					...
					$rows['schema_logo_url'] = $this->p->util->get_th( '<a href="https://developers.google.com/structured-data/customize/logos">'.
						_x( 'Business Logo Image URL', 'option label', 'nextgen-facebook' ).'</a>', null, 'google_schema_logo_url' ).
					...
					$this->p->util->get_th( _x( 'Business Banner (600x60) Image URL',
						'option label', 'nextgen-facebook' ), null, 'google_schema_banner_url' ).
					...
					$this->p->util->get_th( _x( 'Schema Item Type for Home Page',
						'option label', 'nextgen-facebook' ), null, 'google_schema_home_page' ).
					...
					$this->p->util->get_th( _x( 'Schema Item Type by Post Type',
						'option label', 'nextgen-facebook' ), null, 'google_schema_type_for_ptn' ).
					'<td>'.$schema_select.'</td>';
