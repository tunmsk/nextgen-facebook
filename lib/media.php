		...
		public function img_size_within_limits( $img_name, $size_name, $img_width, $img_height, $src_name = '' ) {
			...
			if ( $max_ratio > 0 && $img_ratio >= $max_ratio ) {
				...
				if ( is_admin() ) {
					...
					$this->p->notice->err( sprintf( __( '%1$s image %2$s ignored &mdash; the resulting image of %3$s has an <strong>aspect ratio equal to/or greater than %4$d:1 allowed by the %5$s standard</strong>.', 'nextgen-facebook' ), $src_name, $img_name, $img_width.'x'.$img_height, $max_ratio, $std_name ).' '.$reject_notice, false, true );
				}
				return false;
			}
			...
			if ( ( $min_width > 0 || $min_height > 0 ) &&
				...
				if ( is_admin() ) {
					...
					$reject_notice = $this->p->msgs->get( 'notice-image-rejected', 
						array( 'size_label' => $size_label, 'hard_limit' => true ) );
					$this->p->notice->err( sprintf( __( '%1$s image %2$s ignored &mdash; the resulting image of %3$s is <strong>smaller than the minimum of %4$s allowed by the %5$s standard</strong>.', 'nextgen-facebook' ), $src_name, $img_name, $img_width.'x'.$img_height, $min_width.'x'.$min_height, $std_name ).' '.$reject_notice, false, true );
				}
				return false;
			}
