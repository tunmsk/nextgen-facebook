		...
		public function get_attachment_image_src( $pid, $size_name = 'thumbnail', $check_dupes = true, $force_regen = false ) {
			...
			if ( $this->p->is_avail['media']['ngg'] === true && 
				...
				if ( ! empty( $this->p->mods['media']['ngg'] ) )
					...
					if ( is_admin() )
						$this->p->notice->err( sprintf( __( 'The NextGEN Gallery integration module provided by %1$s is required to read information about image ID %2$s.', 'nextgen-facebook' ), $this->p->cf['plugin'][$this->p->cf['lca']]['short'].' Pro', $pid ) ); 
			...
			if ( empty( $accept_img_size ) ) {
				...
				if ( ( empty( $size_info['crop'] ) && ( ! $is_sufficient_width && ! $is_sufficient_height ) ) ||
					...
					if ( isset( $img_meta['width'] ) && isset( $img_meta['height'] ) &&
						...
						$img_src_txt = __( 'the full size image', 'nextgen-facebook' ).' ('.
							$img_meta['width'].'x'.$img_meta['height'].')';

					} else $img_src_log = $img_src_txt = $img_width.'x'.$img_height;

					if ( $this->p->debug->enabled )
						$this->p->debug->log( 'exiting early: image ID '.$pid.' rejected - '.
							$img_src_log.' too small for the '.$size_name.' ('.
								$size_info['width'].'x'.$size_info['height'].
									( $img_cropped === 0 ? '' : ' cropped' ).') image size' );

					if ( is_admin() )
						$this->p->notice->err( sprintf( __( 'Media Library image ID %1$s has been ignored &mdash; %2$s is too small for the %3$s image size.', 'nextgen-facebook' ), $pid, $img_src_txt, $size_label.' ('.$size_info['width'].'x'.$size_info['height'].( $img_cropped === 0 ? '' : ' '.__( 'cropped', 'nextgen-facebook' ) ).')' ).' '.sprintf( __( 'Upload or choose a larger image, or adjust the <a href="%1%s">%2$s social image dimensions</a>.', 'nextgen-facebook' ), $this->p->util->get_admin_url( 'image-dimensions' ), $size_label ), false, true, $msg_id, true );
				...
				// if this is an open graph image, make sure it is larger than 200x200
					...
					if ( is_admin() )
						$this->p->notice->err( sprintf( __( 'Media Library image ID %1$s has been ignored &mdash; the resulting %2$s image aspect ratio is equal to/or greater than %3$d:1.', 'nextgen-facebook' ), $pid, $img_width.'x'.$img_height, $this->p->cf['head']['max_img_ratio'] ).' '.sprintf( __( 'Upload or choose a larger image, or adjust the <a href="%1%s">%2$s social image dimensions</a>.', 'nextgen-facebook' ), $this->p->util->get_admin_url( 'image-dimensions' ), $size_label ), false, true, $msg_id, true );
