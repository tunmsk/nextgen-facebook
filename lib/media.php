		...
		public function get_attachment_image_src( $pid, $size_name = 'thumbnail', $check_dupes = true, $force_regen = false ) {
			...
			if ( empty( $accept_img_size ) ) {
				...
				if ( ( empty( $size_info['crop'] ) && ( ! $is_sufficient_w && ! $is_sufficient_h ) ) ||
					...
					if ( is_admin() )
						$this->p->notice->err( sprintf( __( '%1$s image ID %2$s ignored &mdash; the resulting image of %3$s is too small for the %4$s image size.', 'nextgen-facebook' ), __( 'Media Library', 'nextgen-facebook' ), $pid, $size_text, '<b>'.$size_label.'</b> ('.$size_info['width'].'x'.$size_info['height'].( $img_cropped === 0 ? '' : ' <i>'.__( 'cropped', 'nextgen-facebook' ).'</i>' ).')' ).' '.$this->p->msgs->get( 'notice-image-rejected', array( 'size_label' => $size_label ) ), false, true, $msg_id, true );
					return self::reset_image_src_info();
						...
						default:
							...
							if ( ( $attr_name == 'src' && $accept_img_size ) ||
								...
								if ( is_admin() ) {
									...
									$this->p->notice->err( sprintf( __( 'Image %1$s in content ignored &mdash; the image width / height is too small for the %2$s image size.', 'nextgen-facebook' ), $og_image['og:image'], '<b>'.$size_label.'</b> ('.$size_name.')' ).$data_wp_pid_msg, false, true, $msg_id, true );
								}
								$og_image = array();
							}
							break;
