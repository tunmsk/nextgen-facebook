		...
		public function img_size_in_bounds( $pid, $size_name, $img_width, $img_height, $media_lib = '' ) {
			...
			if ( $max_ratio > 0 && $img_ratio >= $max_ratio ) {
				...
				if ( is_admin() ) {
					...
					$this->p->notice->err( sprintf( __( '%1$s image ID %2$s ignored &mdash; the resulting image of %3$s has an aspect ratio equal to/or greater than %4$d:1.', 'nextgen-facebook' ), $media_lib, $pid, $img_width.'x'.$img_height, $max_ratio ).' '.$reject_notice, false, true );
				}
			...
			if ( ( $min_width > 0 || $min_height > 0 ) &&
				...
				if ( is_admin() ) {
					$size_label = $this->p->util->get_image_size_label( $size_name );
					$reject_notice = $this->p->msgs->get( 'notice-image-rejected', array( 'size_label' => $size_label ) );
					$this->p->notice->err( sprintf( __( '%1$s image ID %2$s ignored &mdash; the resulting image of %3$s is smaller than the minimum %4$s allowed by the %5$s standard.', 'nextgen-facebook' ), $media_lib, $pid, $img_width.'x'.$img_height, $min_width.'x'.$min_height, $std_name ).' '.$reject_notice, false, true );
				}
