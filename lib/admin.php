	...
	class NgfbAdmin {
		...
		public function licenses_metabox_content( $network = false ) {
			...
			foreach ( NgfbConfig::get_ext_sorted( true ) as $ext => $info ) {
				...
				$links = array();
				$plugin_href = '';
				$details_text = _x( 'Plugin Details', 'plugin action link', 'nextgen-facebook' );
				...
				if ( ! empty( $info['slug'] ) ) {
					...
					if ( is_dir( WP_PLUGIN_DIR.'/'.$info['slug'] ) ) {
						...
						$details_text = _x( 'Plugin Details and Install', 'plugin action link', 'nextgen-facebook' );
					}
					...
					switch ( $action ) {
						case 'plugins_page':
							$html = '<a href="'.$this->iframe_parent_href.'" target="_parent">'.
								sprintf( __( 'Return to %s', 'nextgen-facebook' ), $this->iframe_parent_title ).'</a>';
							break;
						default:
