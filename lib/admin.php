		...
		public function conflict_warnings() {
			...
			if ( $this->p->is_avail['seo']['autodescription'] ) {
				...
				if ( $the_seo_framework->use_og_tags() ) {
					...
					$this->p->notice->err( $err_pre.sprintf( __( 'please uncheck the <strong>Output Open Graph meta tags?</strong> option in <a href="%s">The SEO Framework</a> Social Meta Settings.', 'nextgen-facebook' ), get_admin_url( null, 'admin.php?page=autodescription-settings' ) ) );
				}
				if ( $the_seo_framework->use_facebook_tags() ) {
					if ( $this->p->debug->enabled )
						$this->p->debug->log( $log_pre.'autodescription facebook meta tags are enabled' );
					$this->p->notice->err( $err_pre.sprintf( __( 'please uncheck the <strong>Output Facebook meta tags?</strong> option in <a href="%s">The SEO Framework</a> Social Meta Settings.', 'nextgen-facebook' ), get_admin_url( null, 'admin.php?page=autodescription-settings' ) ) );
				}
				if ( $the_seo_framework->use_twitter_tags() ) {
					if ( $this->p->debug->enabled )
						$this->p->debug->log( $log_pre.'autodescription twitter meta tags are enabled' );
					$this->p->notice->err( $err_pre.sprintf( __( 'please uncheck the <strong>Output Twitter meta tags?</strong> option in <a href="%s">The SEO Framework</a> Social Meta Settings.', 'nextgen-facebook' ), get_admin_url( null, 'admin.php?page=autodescription-settings' ) ) );
				}
