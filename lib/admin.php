	...
	class NgfbAdmin {
		...
		public function load_setting_page() {
			...
			if ( ! self::$pkg[$lca]['aop'] ) {
				add_meta_box( $this->pagehook.'_purchase',
					_x( 'Pro Version Available', 'metabox title (side)', 'nextgen-facebook' ),
		...
		public function show_metabox_version_info() {
			...
			foreach ( $this->p->cf['plugin'] as $ext => $info ) {
				...
				echo '<tr><th class="side">'._x( 'Installed', 'side label', 'nextgen-facebook' ).':</th>
					<td class="side-version" '.$installed_style.'>'.$installed_version.'</td></tr>';

				echo '<tr><th class="side">'._x( 'Stable', 'side label', 'nextgen-facebook' ).':</th>
					<td class="side-version">'.$stable_version.'</td></tr>';

				echo '<tr><th class="side">'._x( 'Latest', 'side label', 'nextgen-facebook' ).':</th>
			...
			foreach ( NgfbConfig::get_ext_sorted( true ) as $ext => $info ) {
				...
				if ( ! empty( $info['slug'] ) &&
					...
					if ( is_dir( WP_PLUGIN_DIR.'/'.$info['slug'] ) ) {
						...
						$view_text = _x( 'Plugin Details + Install', 'plugin action link', 'nextgen-facebook' );
					}
				...
				if ( ! empty( $info['url']['latest'] ) ) {
					$links[] = '<a href="'.$info['url']['latest'].'" tabindex="'.++$tabindex.'">'.
						_x( 'Download Latest', 'plugin action link', 'nextgen-facebook' ).'</a> (ZIP)';
				}
		...
		public function conflict_warnings() {
			...
			foreach ( $this->p->cf['php']['extensions'] as $php_ext => $php_label ) {
				if ( ! extension_loaded( $php_ext ) ) {
					...
					$this->p->notice->err( sprintf( __( 'The PHP <a href="%1$s" target="_blank"> %2$s extension</a> is not loaded.', 'nextgen-facebook' ), 'https://secure.php.net/manual/en/book.'.$php_ext.'.php', $php_label ).' '.__( 'Please contact your hosting provider to have the missing PHP extension installed and/or enabled.', 'nextgen-facebook' ) );
				}
			}
