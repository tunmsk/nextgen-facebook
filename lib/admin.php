				...
				foreach ( $info['lib']['pro'] as $sub => $libs ) {
					...
					foreach ( $libs as $id_key => $label ) {
						...
						$features[$label] = array( 
							...
							'tooltip' => sprintf( __( 'If the %1$s plugin is detected, %2$s will load additional integration modules to provide enhanced support and features for %3$s.', 'nextgen-facebook'), $label, $short_pro, $label ),
							'td_class' => $aop ? '' : 'blank',
						);
					}
				}
