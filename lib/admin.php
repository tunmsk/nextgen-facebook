	...
	class NgfbAdmin {
					...
					switch ( $action_name ) {
						case 'check_for_updates': 
							...
							if ( $this->p->is_avail['util']['um'] ) {
								...
								if ( isset( $ngfbum->update ) )
									$ngfbum->update->check_for_updates( null, true, false );	// $use_cache = false
								else $this->p->notice->err( sprintf( __( 'The <b>%2$s</b> extension is not initialized properly. Please make sure you are using the latest versions of %1$s and %2$s.', 'nextgen-facebook' ), $info['name'], $um_info['name'] ) );
							} else $this->p->notice->err( sprintf( __( 'The <b>%1$s</b> extension must be active in order to check for Pro version updates.', 'nextgen-facebook' ), $um_info['name'] ) );
							break;
			...
			foreach ( $this->p->cf['plugin'] as $ext => $info ) {
				...
				if ( ! empty( $info['url']['support'] ) && self::$pkg[$ext]['aop'] )
					$help_links .= '<li>'.sprintf( __( 'Open a <a href="%s" target="_blank">Priority Support Ticket</a> (Pro version)',
						'nextgen-facebook' ), $info['url']['support'] ).'</li>'."\n";
		...
		public function conflict_warnings() {
			...
			if ( ! extension_loaded( 'curl' ) ) {
				...
				$this->p->notice->err( sprintf( __( '<a href="%s" target="_blank">PHP Client URL Library (cURL) extension</a> is not loaded.', 'nextgen-facebook' ), 'http://php.net/manual/en/book.curl.php' ).' '.__( 'please contact your hosting provider to have the missing PHP extension installed and/or enabled.', 'nextgen-facebook' ) );
			}
			if ( ! extension_loaded( 'json' ) ) {
				if ( $this->p->debug->enabled )
					$this->p->debug->log( 'php json extension is not loaded' );
				$this->p->notice->err( sprintf( __( '<a href="%s" target="_blank">PHP JavaScript Object Notation (JSON) extension</a> is not loaded.', 'nextgen-facebook' ), 'http://php.net/manual/en/book.json.php' ).' '.__( 'please contact your hosting provider to have the missing PHP extension installed and/or enabled.', 'nextgen-facebook' ) );
			}
			if ( ! extension_loaded( 'mbstring' ) ) {
				if ( $this->p->debug->enabled )
					$this->p->debug->log( 'php mbstring extension is not loaded' );
				$this->p->notice->err( sprintf( __( '<a href="%s" target="_blank">PHP Multibyte String extension</a> is not loaded.', 'nextgen-facebook' ), 'http://php.net/manual/en/book.mbstring.php' ).' '.__( 'please contact your hosting provider to have the missing PHP extension installed and/or enabled.', 'nextgen-facebook' ) );
			}
			...
			if ( $this->p->is_avail['seo']['autodescription'] ) {
				...
				if ( $the_seo_framework->use_og_tags() ) {
					...
					$this->p->notice->err( $err_pre.sprintf( __( 'please uncheck the <strong>%1$s</strong> option in <a href="%2$s">The SEO Framework</a> Social Meta Settings.', 'nextgen-facebook' ), 'Output Open Graph meta tags?', get_admin_url( null, 'admin.php?page=autodescription-settings' ) ) );
				}
				if ( $the_seo_framework->use_facebook_tags() ) {
					if ( $this->p->debug->enabled )
						$this->p->debug->log( $log_pre.'autodescription facebook meta tags are enabled' );
					$this->p->notice->err( $err_pre.sprintf( __( 'please uncheck the <strong>%1$s</strong> option in <a href="%2$s">The SEO Framework</a> Social Meta Settings.', 'nextgen-facebook' ), 'Output Facebook meta tags?', get_admin_url( null, 'admin.php?page=autodescription-settings' ) ) );
				}
				if ( $the_seo_framework->use_twitter_tags() ) {
					if ( $this->p->debug->enabled )
						$this->p->debug->log( $log_pre.'autodescription twitter meta tags are enabled' );
					$this->p->notice->err( $err_pre.sprintf( __( 'please uncheck the <strong>%1$s</strong> option in <a href="%2$s">The SEO Framework</a> Social Meta Settings.', 'nextgen-facebook' ), 'Output Twitter meta tags?', get_admin_url( null, 'admin.php?page=autodescription-settings' ) ) );
				}
				if ( $the_seo_framework->is_option_checked( 'knowledge_output' ) ) {
					...
					$this->p->notice->err( $err_pre.sprintf( __( 'please uncheck the <strong>Output Knowledge tags?</strong> option in <a href="%s">The SEO Framework</a> Knowledge Graph Settings.', 'nextgen-facebook' ), get_admin_url( null, 'admin.php?page=autodescription-settings' ) ) );
				}
				foreach ( array(
					...
					if ( $the_seo_framework->get_option( $key ) ) {
						...
						$this->p->notice->err( $err_pre.sprintf( __( 'please uncheck the <strong>%1$s</strong> option in <a href="%2$s">The SEO Framework</a> Social Meta Settings.', 'nextgen-facebook' ), $label, get_admin_url( null, 'admin.php?page=autodescription-settings' ) ) );
					}
				}
			}
		...
		public function required_notices() {
			...
			if ( $have_ext_tid === true ) {
				if ( $this->p->is_avail['util']['um'] &&
					...
					if ( version_compare( $this->p->cf['plugin']['ngfbum']['version'], $min_version, '<' ) )
		...
		public function modify_tmpl_head_attributes() {
			...
			if ( empty( $header_files ) ) {
				$this->p->notice->err( __( 'No header templates found in the parent or child theme directories.', 'nextgen-facebook' ) );
				return;	// exit early
			}
			...
			foreach ( $header_files as $tmpl_file ) {
				...
				if ( ( $html = SucomUtil::get_stripped_php( $tmpl_file ) ) === false ||
					...
					$this->p->notice->err( sprintf( __( 'No &lt;head&gt; HTML element found in the %s template.', 'nextgen-facebook' ), $tmpl_file ) );
					continue;
				}

				// make a backup of the original
				if ( ! copy( $tmpl_file, $backup_file ) ) {
					$this->p->notice->err( sprintf( __( 'Error copying %1$s to %2$s.', 'nextgen-facebook' ), $tmpl_file, $backup_base ) );
					continue;
				}
				...
				if ( ! $tmpl_fh = @fopen( $tmpl_file, 'wb' ) ) {
					$this->p->notice->err( sprintf( __( 'Failed to open %s for writing.', 'nextgen-facebook' ), $tmpl_file ) );
					continue;
				}
		...
		public function get_site_use( &$form, $network = false, $name, $force = false ) {
			if ( $network !== true )
				return '';
			return $form->get_th_html( _x( 'Site Use', 'option label (very short)', 'nextgen-facebook' ), 'site_use' ).
