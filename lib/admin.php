					...
					$features = array(
						...
						'(tool) WordPress Object (Non-Persistant) Cache' => array(
							'status' => $this->p->is_avail['cache']['object'] ?
								'on' : ( SucomUtil::get_const( 'NGFB_OBJECT_CACHE_DISABLE' ) ?
									'off' : 'rec' ),
						),
						'(tool) WordPress Transient (Persistant) Cache' => array(
							'status' => $this->p->is_avail['cache']['transient'] ?
								'on' : ( SucomUtil::get_const( 'NGFB_TRANSIENT_CACHE_DISABLE' ) ?
									'off' : 'rec' ),
						),
		...
		public function conflict_warnings() {
			...
			if ( $this->p->is_avail['seo']['autodescription'] ) {
				...
				if ( $the_seo_framework->use_og_tags() ) {
					...
					$this->p->notice->err( $err_pre.sprintf( __( 'please uncheck the \'<em>Output Open Graph meta tags?</em>\' option in <a href="%s">The SEO Framework</a> Social Meta Settings.', 'nextgen-facebook' ), get_admin_url( null, 'admin.php?page=autodescription-settings' ) ) );
				}
				if ( $the_seo_framework->use_facebook_tags() ) {
					if ( $this->p->debug->enabled )
						$this->p->debug->log( $log_pre.'autodescription facebook meta tags are enabled' );
					$this->p->notice->err( $err_pre.sprintf( __( 'please uncheck the \'<em>Output Faceboook meta tags?</em>\' option in <a href="%s">The SEO Framework</a> Social Meta Settings.', 'nextgen-facebook' ), get_admin_url( null, 'admin.php?page=autodescription-settings' ) ) );
				}
				if ( $the_seo_framework->use_twitter_tags() ) {
					if ( $this->p->debug->enabled )
						$this->p->debug->log( $log_pre.'autodescription twitter meta tags are enabled' );
					$this->p->notice->err( $err_pre.sprintf( __( 'please uncheck the \'<em>Output Twitter meta tags?</em>\' option in <a href="%s">The SEO Framework</a> Social Meta Settings.', 'nextgen-facebook' ), get_admin_url( null, 'admin.php?page=autodescription-settings' ) ) );
				}
				if ( $the_seo_framework->is_option_checked( 'knowledge_output' ) ) {
					...
					$this->p->notice->err( $err_pre.sprintf( __( 'please uncheck the \'<em>Output Knowledge tags?</em>\' option in <a href="%s">The SEO Framework</a> Knowledge Graph Settings.', 'nextgen-facebook' ), get_admin_url( null, 'admin.php?page=autodescription-settings' ) ) );
				}
				foreach ( array(
					...
					if ( $the_seo_framework->get_option( $key ) ) {
						...
						$this->p->notice->err( $err_pre.sprintf( __( 'please uncheck the \'<em>%1$s</em>\' option in <a href="%2$s">The SEO Framework</a> Social Meta Settings.', 'nextgen-facebook' ), $label, get_admin_url( null, 'admin.php?page=autodescription-settings' ) ) );
					}
				}
			}
		}
