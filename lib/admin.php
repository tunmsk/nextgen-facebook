...
if ( ! class_exists( 'NgfbAdmin' ) ) {
	...
	class NgfbAdmin {
		...
		public function override_textdomain_mofile( $wp_mofile, $domain ) {
			if ( $domain === 'nextgen-facebook' ) {
				$plugin_mofile = NGFB_PLUGINDIR.'languages/'.basename( $wp_mofile );
		...
		public function add_plugin_action_links( $links, $file ) {
			...
			if ( ! empty( $info['url']['latest'] ) )
				$links[] = '<a href="'.$info['url']['latest'].'">'.
					_x( 'Latest', 'plugin action link', 'nextgen-facebook' ).'</a>';

			if ( ! empty( $info['url']['support'] ) && self::$pkg[$ext]['aop'] )
				$links[] = '<a href="'.$info['url']['support'].'">'.
					_x( 'Support', 'plugin action link', 'nextgen-facebook' ).'</a>';
			elseif ( ! empty( $info['url']['forum'] ) )
				$links[] = '<a href="'.$info['url']['forum'].'">'.
					_x( 'Forum', 'plugin action link', 'nextgen-facebook' ).'</a>';

			/*
			if ( ! empty( $info['url']['review'] ) ) {
				$links[] = '<a href="'.$info['url']['latest'].'">'.
					_x( 'Rating', 'plugin action link', 'nextgen-facebook' ).'</a>';
			}
			*/
		...
		public function licenses_metabox_content( $network = false ) {
			...
			foreach ( NgfbConfig::get_ext_sorted( true ) as $ext => $info ) {
				...
				$links = array();
				$plugin_href = '';
				$view_text = _x( 'Plugin Details', 'plugin action link', 'nextgen-facebook' );
				...
				if ( ! empty( $info['slug'] ) && 
					...
					} else $view_text = _x( 'Plugin Details + Install', 'plugin action link', 'nextgen-facebook' );
