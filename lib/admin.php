					...
					foreach ( $libs as $id => $name ) {
						...
						$features[$name] = array( 
							'status' => class_exists( $lca.'pro'.$sub.$id ) ?
								( $lca_aop ? 'on' : $off ) : $off,
							'tooltip' => sprintf( __( 'If the %1$s plugin is detected, %2$s will load additional integration modules to provide enhanced support and features for %3$s.', 'nextgen-facebook'), $name, $short_pro, $name ),
