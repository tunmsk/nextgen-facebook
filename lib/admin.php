	...
	class NgfbAdmin {
		...
		public function timed_notices( $store = false ) {
			...
			foreach ( $this->p->cf['plugin'] as $ext => $info ) {
				...
				if ( ! isset( $dis_arr[$type.'_'.$msg_id_works] ) && 
					...
					$this->p->notice->log( $type, '<b>'.__( 'Excellent!', 'nextgen-facebook' ).'</b> '.sprintf( __( 'It looks like you\'ve been running <b>%1$s version %2$s</b> for several days &mdash; is it working well with <b>WordPress version %3$s</b>?', 'nextgen-facebook' ), $info['name'], $info['version'], $wp_version ).'<ul><li><a href="https://wordpress.org/plugins/'.$info['slug'].'/?compatibility[version]='.$wp_version.'&compatibility[topic_version]='.$info['version'].'&compatibility[compatible]=1" target="_blank">'.__( 'Let us know with a "Works" vote on WordPress.org!', 'nextgen-facebook' ).'</a></li>'.$help_links.'</ul>', $store, $user_id, $msg_id_works, true, array( 'label' => false ) );

				} elseif ( ! isset( $dis_arr[$type.'_'.$msg_id_review] ) && 
					isset( $ts[$ext.'_install_time'] ) && 
						$ts[$ext.'_install_time'] < $weeks ) {

					$this->p->notice->log( $type, '<b>'.__( 'Fantastic!', 'nextgen-facebook' ).'</b> '.sprintf( __( 'It looks like you\'ve been running <b>%s</b> for several weeks &mdash; how do you like it so far?', 'nextgen-facebook' ), $info['name'] ).'<ul><li><a href="'.$info['url']['review'].'" target="_blank">'.__( 'Let us know with a 5-star rating and a few encouraging words on wordpress.org!', 'nextgen-facebook' ).'</a> ;-)</li>'.$help_links.'</ul>', $store, $user_id, $msg_id_review, true, array( 'label' => false ) );
				}
			}
		}
		...
		public function head_attr_filter_update() {
			...
			if ( ( $html = SucomUtil::get_stripped_php( $file ) ) === false ||
				...
				$this->p->notice->err( sprintf( __( 'No update possible - an un-modified &lt;head&gt; element was not found in %s.', 'nextgen-facebook' ), $file ), true );
				return;
			}
