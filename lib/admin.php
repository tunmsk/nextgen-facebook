...
if ( ! class_exists( 'NgfbAdmin' ) ) {
	...
	class NgfbAdmin {
		...
		public function modify_tmpl_head_attributes() {
			...
			if ( empty( $headers ) ) {
				$this->p->notice->err( sprintf( __( 'No header templates found in %s.', 'nextgen-facebook' ), get_stylesheet_directory() ) );
			} else {
				foreach ( $headers as $file ) {
					$base = basename( $file );
					...
					if ( ( $html = SucomUtil::get_stripped_php( $file ) ) === false ||
						...
						$this->p->notice->err( sprintf( __( '&lt;head&gt; element not found in %s.', 'nextgen-facebook' ), $file ) );
						continue;
					}
	
					// make a backup of the original
					if ( ! copy( $file, $backup ) ) {
						$this->p->notice->err( sprintf( __( 'Error copying %1$s to %2$s.', 'nextgen-facebook' ), $base, $backup ) );
						continue;
					}
					...
					if ( ! $tmpl_fh = @fopen( $file, 'wb' ) ) {
						$this->p->notice->err( sprintf( __( 'Failed to open %s for writing.', 'nextgen-facebook' ), $file ) );
						continue;
					}
					...
					if ( fwrite( $tmpl_fh, $tmpl_contents ) ) {
						...
						$this->p->notice->err( sprintf( __( 'Failed to write the %1$s template. You may need to restore the original template from %2$s.', 'nextgen-facebook' ), $base, $backup ) );
					}
