	...
	class NgfbSchema {
		...
		public static function add_single_organization_data( &$json_data, $mod, $org_id = 'site', $logo_key = 'org_logo_url', $list_element = false ) {
			...
			if ( $ngfb->debug->enabled ) {
				$ngfb->debug->log( 'adding single organization data for '.$org_id );
			}

			$org_opts = apply_filters( $ngfb->cf['lca'].'_get_organization_options', false, $mod, $org_id );	// returns localized values
			...
			if ( empty( $org_opts ) ) {	// $org_opts can be false or empty array
				...
				$ngfb->debug->log( 'using custom organization options for '.$org_id );
			}
			...
			if ( empty( $list_element ) && ( $org_type_url = self::get_data_type_url( $json_data ) ) !== false ) {
				$org_type_id = false;
				if ( $ngfb->debug->enabled ) {
					$ngfb->debug->log( 'using inherited schema type url = '.$org_type_url );
				}
			} else {
				...
				$org_type_url = $ngfb->schema->get_schema_type_url( $org_type_id, 'organization' );
			}

			if ( $ngfb->debug->enabled ) {
				$ngfb->debug->log( 'org_type_id = '.$org_type_id );
				$ngfb->debug->log( 'org_type_url = '.$org_type_url );
			}
			...
			if ( ! empty( $logo_key ) ) {
				...
				if ( empty( $ret['logo'] ) ) {
					...
					if ( $ngfb->notice->is_admin_pre_notices() && 
			...
			if ( ! empty( $org_type_id ) && $org_type_id !== 'organization' && 
				$ngfb->schema->is_schema_type_child_of( $org_type_id, 'local.business' ) ) {
		...
		public static function get_site_organization( $mixed = 'current' ) {
			...
			foreach ( apply_filters( $ngfb->cf['lca'].'_social_accounts', 
				$ngfb->cf['form']['social_accounts'] ) as $social_key => $social_label ) {

				$url = SucomUtil::get_locale_opt( $social_key, $ngfb->options, $mixed );	// localized value
			...
			return array(
				...
				'org_logo_url' => SucomUtil::get_locale_opt( 'schema_logo_url', $ngfb->options, $mixed ),	// localized value
				'org_banner_url' => SucomUtil::get_locale_opt( 'schema_banner_url', $ngfb->options, $mixed ),	// localized value
		...
		public static function add_single_place_data( &$json_data, $mod, $place_id = false, $list_element = false ) {
			...
			if ( $ngfb->debug->enabled ) {
				$ngfb->debug->log( 'adding single place data for '.$place_id );
			}

			$place_opts = apply_filters( $ngfb->cf['lca'].'_get_place_options', false, $mod, $place_id );
			...
			if ( ! $list_element && ( $place_type_url = self::get_data_type_url( $json_data ) ) !== false ) {
				if ( $ngfb->debug->enabled ) {
					$ngfb->debug->log( 'using inherited schema type url = '.$place_type_url );
				}
			} else {
				...
				$place_type_url = $ngfb->schema->get_schema_type_url( $place_type_id, 'place' );
			}
			...
			$ret = apply_filters( $ngfb->cf['lca'].'_json_data_single_place', $ret, $mod, $place_id );
		...
		public static function add_single_event_data( &$json_data, $mod, $event_id = false, $list_element = false ) {
			...
			if ( $ngfb->debug->enabled ) {
				$ngfb->debug->log( 'adding single event data for '.$event_id );
			}

			$event_opts = apply_filters( $ngfb->cf['lca'].'_get_event_options', false, $mod, $event_id );
			...
			if ( ! $list_element && ( $event_type_url = self::get_data_type_url( $json_data ) ) !== false ) {
				if ( $ngfb->debug->enabled ) {
					$ngfb->debug->log( 'using inherited schema type url = '.$event_type_url );
				}
			} else {
				$event_type_id = empty( $event_opts['event_type'] ) ? 'event' : $event_opts['event_type'];
				$event_type_url = $ngfb->schema->get_schema_type_url( $event_type_id, 'event' );
			}
			...
			$ret = apply_filters( $ngfb->cf['lca'].'_json_data_single_event', $ret, $mod, $event_id );
		...
		public static function add_single_person_data( &$json_data, $mod, $user_id, $list_element = true ) {
			...
			$ngfb =& Ngfb::get_instance();
			if ( $ngfb->debug->enabled ) {
				$ngfb->debug->log( 'adding single person data for '.$user_id );
			}

			$person_opts = apply_filters( $ngfb->cf['lca'].'_get_person_options', false, $mod, $user_id );
			...
			if ( empty( $person_opts ) ) {	// $person_opts could be false or empty array
				...
				foreach ( NgfbUser::get_user_id_contact_methods( $user_id ) as $cm_id => $cm_label ) {
					...
					if ( $cm_id === $ngfb->options['plugin_cm_twitter_name'] ) {
				...
				$person_opts = array(
					...
					'person_og_image' => $user_mod['obj']->get_og_image( 1, $ngfb->cf['lca'].'-schema', $user_id, false ),
			...
			if ( ! $list_element && ( $person_type_url = self::get_data_type_url( $json_data ) ) !== false ) {
				if ( $ngfb->debug->enabled ) {
					$ngfb->debug->log( 'using inherited schema type url = '.$person_type_url );
				}
			} else {
				$person_type_id = empty( $person_opts['person_type'] ) ? 'person' : $person_opts['person_type'];	// person or patient
				$person_type_url = $ngfb->schema->get_schema_type_url( $person_type_id, 'person' );
			}
			 ...
			 * Google Knowledge Graph
			 */
			$person_opts['person_sameas'] = apply_filters( $ngfb->cf['lca'].'_json_data_single_person_sameas',
			...
			$ret = apply_filters( $ngfb->cf['lca'].'_json_data_single_person', $ret, $mod, $user_id );
		...
		public static function add_single_image_data( &$json_data, $opts, $prefix = 'og:image', $list_element = true ) {
			...
			if ( ! $list_element && ( $image_type_url = self::get_data_type_url( $json_data ) ) !== false ) {
				if ( $ngfb->debug->enabled ) {
					$ngfb->debug->log( 'using inherited schema type url = '.$image_type_url );
				}
			} else {
				...
				if ( isset( $assoc[$key_name] ) && $assoc[$key_name] !== '' ) {	// exclude empty strings
					if ( isset( $json_data[$itemprop_name] ) && empty( $overwrite ) ) {
						...
						if ( $ngfb->debug->enabled ) {
							...
							$ngfb->debug->log( 'added itemprop '.$itemprop_name.' = '.print_r( $json_data[$itemprop_name], true ) );
						}
						$itemprop_added++;
					}
				}
