	...
	class NgfbSchema {
		...
		public static function add_single_organization_data( &$json_data, &$mod, $logo_key = 'schema_logo_url', $list_element = false ) {
			...
			$ret = array(
				...
				'name' => $ngfb->og->get_site_name( $mod['use_post'] ),
			);

			$desc = $ngfb->og->get_site_desc( $mod['use_post'] );
			if ( ! empty( $desc ) )
				$ret['description'] = $desc;
		...
		public static function add_single_person_data( &$json_data, $user_id = 0, $list_element = true ) {
			...
			$name = $mod['obj']->get_author_name( $user_id, $ngfb->options['schema_author_name'] );
			if ( ! empty( $name ) )
				$ret['name'] = $name;

			$desc = $ngfb->util->get_mod_options( $user_id, $mod['name'], array( 'schema_desc', 'og_desc' ) );
			if ( empty( $desc ) )
