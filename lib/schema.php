...
if ( ! class_exists( 'NgfbSchema' ) ) {
	...
	class NgfbSchema {
		...
		public static function add_single_organization_data( &$json_data, $mod, $org_id = 'site', $logo_key = 'org_logo_url', $list_element = false ) {
			...
			$org_opts = apply_filters( $ngfb->cf['lca'].'_get_organization_options', false, $mod, $org_id );
			...
			if ( isset( $org_opts['org_place_id'] ) && $org_opts['org_place_id'] !== 'none' ) {
				...
				if ( self::add_single_place_data( $ret['location'], $mod, $place_id, false ) ) {	// $list_element = false
					 ...
					 */
					if ( $org_type_id && $ngfb->schema->is_schema_type_child_of( $org_type_id, 'local.business' ) ) {
						$this->promote_location_property( $ret );
					}
				} else {
		...
		public static function get_site_organization( $mixed = 'current' ) {
			...
			foreach ( apply_filters( $ngfb->cf['lca'].'_social_accounts', 
				...
				$url = SucomUtil::get_locale_opt( $social_key, $ngfb->options, $mixed );
			...
			return array(
				...
				'org_url' => SucomUtil::get_site_url( $ngfb->options, $mixed ),
				'org_name' => SucomUtil::get_site_name( $ngfb->options, $mixed ),
				'org_alt_name' => SucomUtil::get_site_alt_name( $ngfb->options, $mixed ),
				'org_desc' => SucomUtil::get_site_description( $ngfb->options, $mixed ),
