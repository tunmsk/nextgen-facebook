...
if ( ! class_exists( 'NgfbSchema' ) ) {
	...
	class NgfbSchema {
		...
		public static function add_single_organization_data( &$json_data, $mod, $org_id = 'site', $logo_key = 'org_logo_url', $list_element = false ) {
			...
			$opts = apply_filters( $ngfb->cf['lca'].'_get_organization_options', false, $mod, $org_id );

			if ( empty( $opts ) ) {	// $opts can be false or empty array
				if ( $ngfb->debug->enabled )
					$ngfb->debug->log( 'using default organization options for '.$org_id );
			 ...
			 */
			$opts['org_sameas'] = apply_filters( $ngfb->cf['lca'].'_json_data_single_organization_sameas',
		...
		public static function get_site_organization( $mixed = 'current' ) {
			...
			foreach ( apply_filters( $ngfb->cf['lca'].'_social_accounts', 
				$ngfb->cf['form']['social_accounts'] ) as $key => $label ) {

				$url = SucomUtil::get_locale_opt( $key, $ngfb->options, $mixed );
				if ( empty( $url ) )
					continue;
				elseif ( $key === 'tc_site' )
		...
		public static function add_single_place_data( &$json_data, $mod, $place_id = false, $list_element = false ) {
			...
			$opts = apply_filters( $ngfb->cf['lca'].'_get_place_options', false, $mod, $place_id );
			...
			if ( ! empty( $opts['place_business_type'] ) &&
				...
				if ( $ngfb->schema->is_schema_type_child_of( $opts['place_business_type'], 'food.establishment' ) ) {
					foreach ( array(
		...
		public static function add_single_event_data( &$json_data, $mod, $event_id = false, $list_element = false ) {
			...
			$opts = apply_filters( $ngfb->cf['lca'].'_get_event_options', false, $mod, $event_id );
		...
		public static function add_single_person_data( &$json_data, $mod, $user_id, $list_element = true ) {
			...
			$opts = apply_filters( $ngfb->cf['lca'].'_get_person_options', false, $mod, $user_id );
			...
			if ( $ngfb->debug->enabled )
				$ngfb->debug->log_arr( 'person options', $opts );
			 ...
			 * Google Knowledge Graph
			 */
			$opts['person_sameas'] = apply_filters( $ngfb->cf['lca'].'_json_data_single_person_sameas',
