	...
	class NgfbSchema {
		...
		public function filter_json_data_http_schema_org_person( $json_data, 
			...
			if ( is_front_page() || $main_entity ) {
				...
				if ( is_front_page() ) {
					// add the sameAs social profile links
					foreach ( NgfbUser::get_user_id_contact_methods( $author_id ) as $cm_id => $cm_label ) {
		...
		public static function add_single_organization_data( &$json_data,
			...
			$ret = array(
				...
				'name' => $ngfb->og->get_site_name( $post_id ),
			);

			$desc = $ngfb->og->get_site_desc( $post_id );
			if ( ! empty( $desc ) )
				$ret['description'] = $desc;

			if ( ! empty( $ngfb->options[$logo_key] ) )
				self::add_single_image_data( $ret['logo'],
					$ngfb->options, $logo_key, false );	// list_element = false
		...
		public static function add_single_person_data( &$json_data, $author_id, $list_element = true ) {
			...
			if ( empty( $author_id ) ) {
				if ( $ngfb->debug->enabled )
					$ngfb->debug->log( 'exiting early: empty author_id' );
				return false;
			}
			...
			$desc = $ngfb->util->get_mod_options( 'user', $author_id, 
		...
		public static function add_single_image_data( &$json_data,
			...
			if ( empty( $opts[$opt_pre] ) && empty( $opts[$opt_pre.':secure_url'] ) ) {
				if ( $ngfb->debug->enabled )
					$ngfb->debug->log( 'exiting early: '.$opt_pre.' and '.
