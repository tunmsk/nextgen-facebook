	...
	class NgfbSchema {
		...
		public static function add_single_organization_data( &$json_data, &$mod, $logo_key = 'schema_logo_url', $list_element = false ) {
			...
			$ret = array(
				...
				'name' => SucomUtil::get_site_name( $ngfb->options, $mod ),
			);

			if ( ! empty( $ngfb->options['schema_alt_name'] ) )
				$ret['alternateName'] = $ngfb->options['schema_alt_name'];

			$desc = SucomUtil::get_site_description( $ngfb->options, $mod );
			if ( ! empty( $desc ) )
				$ret['description'] = $desc;
