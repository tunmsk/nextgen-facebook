	...
	class NgfbSchema {
			...
			if ( empty( $org_opts ) ) {	// $org_opts could be false or empty array
				...
				foreach ( apply_filters( $ngfb->cf['lca'].'_social_accounts', 
					$ngfb->cf['form']['social_accounts'] ) as $key => $label )
						$org_opts['org_sameas_'.$key] = SucomUtil::get_locale_opt( $key, $ngfb->options, $mod );
			}
		...
		public static function add_single_place_data( &$json_data, &$mod, $place_id = false, $list_element = false ) {
			...
			if ( ! empty( $place_opts['place_business_type'] ) &&
				...
				if ( $ngfb->schema->schema_type_child_of( $place_opts['place_business_type'], 'food.establishment' ) ) {
					foreach ( array(
						'menu' => 'place_menu_url',
		...
		public static function add_single_person_data( &$json_data, $user_id = 0, $list_element = true ) {
			...
			if ( empty( $user_id ) ) {
				...
				return 0;
			} else $mod = $ngfb->m['util']['user']->get_mod( $user_id );
			...
			$name = $mod['obj']->get_author_meta( $user_id, $ngfb->options['schema_author_name'] );
			if ( ! empty( $name ) )
				$ret['name'] = $name;
