		...
		public function load_meta_page( $screen = false ) {
			...
			if ( $post_obj->post_status === 'auto-draft' ) {
				...
				if ( apply_filters( $lca.'_add_metabox_post', $add_metabox, $post_id, $post_obj->post_type ) ) {
					...
					if ( ! empty( NgfbMeta::$head_meta_info['schema:type:id'] ) )
						$this->update_sortable_meta( $mod, 'schema_id', NgfbMeta::$head_meta_info['schema:type:id'] );
						...
						foreach( $metas[$tag] as $m ) {
							foreach( $types as $t ) {
								if ( isset( $m[$t] ) && $m[$t] !== 'generator' && 
									...
									$conflicts_found++;
									$this->p->notice->err( sprintf( __( 'Possible conflict detected &mdash; your theme or another plugin is adding a <code>%1$s</code> HTML tag to the head section of this webpage.', 'nextgen-facebook' ), $tag.' '.$t.'="'.$m[$t].'"' ) );
								}
							}
						}
					}
			...
			switch ( get_post_status( $post_id ) ) {
				...
				case 'publish':
					...
					$transients = array(
						...
						'NgfbMeta::get_mod_column_content' => array( $cache_salt ),
