...
if ( ! class_exists( 'NgfbPost' ) ) {
	...
	class NgfbPost extends NgfbMeta {
		...
		public function check_post_head_duplicates( $post_id = true, $post_obj = false ) {
			...
			$exec_count = (int) get_option( NGFB_POST_CHECK_NAME );	// changes false to 0
						...
						foreach( $metas[$tag] as $meta ) {
							foreach( $types as $type ) {
								if ( isset( $meta[$type] ) && $meta[$type] !== 'generator' && 
									...
									$conflicts_found++;
									$this->p->notice->err( sprintf( __( 'Possible conflict detected &mdash; your theme or another plugin is adding a <code>%1$s</code> HTML tag to the head section of this webpage.', 'nextgen-facebook' ), $tag.' '.$type.'="'.$meta[$type].'"' ) );
								}
							}
						}
					}
