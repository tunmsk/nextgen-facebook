...
if ( ! class_exists( 'NgfbPost' ) ) {
	...
	class NgfbPost extends NgfbMeta {
		...
		public function check_post_header( $post_id = true, &$post_obj = false ) {
			...
			if ( current_user_can( 'manage_options' ) &&
				$this->p->check->aop( $lca, true, $this->p->is_avail['aop'] ) )
					$notice_suffix = ' ('.sprintf( __( 'see <a href="%s">WP / Theme Integration</a> settings',
						'nextgen-facebook' ), $this->p->util->get_admin_url( 'advanced#sucom-tabset_plugin-tab_integration' ) ).')';
			else $notice_suffix = '';
			...
			if ( ! $conflicts_found )
				$this->p->notice->inf( __( 'Awesome! No duplicate meta tags found. :-)', 'nextgen-facebook' ) );
