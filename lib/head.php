		...
		private function get_single_tag( $tag = 'meta', $type = 'property', $name, $value = '', $comment = '', $use_post = false ) {
			...
			foreach ( $ret as $num => $parts ) {
				if ( defined( 'NGFB_FILTER_SINGLE_TAGS' ) && NGFB_FILTER_SINGLE_TAGS ) {
					/*
					 * Example: 'ngfb_link_rel_publisher_content'
					 * apply_filters( 'ngfb_link_rel_'.$name.'_content', $value, $comment, $use_post );
					 *
					 * Example: 'ngfb_meta_itemprop_description_content'
					 * apply_filters( 'ngfb_meta_itemprop_'.$name.'_content', $value, $comment, $use_post );
					 *
					 * Example: 'ngfb_meta_name_twitter:description_content'
					 * apply_filters( 'ngfb_meta_name_'.$name.'_content', $value, $comment, $use_post );
					 *
					 * Example: 'ngfb_meta_property_og:description_content'
					 * apply_filters( 'ngfb_meta_property_'.$name.'_content', $value, $comment, $use_post );
					 */
