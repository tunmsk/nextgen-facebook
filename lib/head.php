...
if ( ! class_exists( 'NgfbHead' ) ) {
	...
	class NgfbHead {
		...
		public function __construct( &$plugin ) {
			...
			add_action( 'wp_head', array( &$this, 'add_head' ), NGFB_HEAD_PRIORITY );
			add_action( 'amp_post_template_head', array( &$this, 'add_head' ), NGFB_HEAD_PRIORITY );
			...
			if ( strpos( $this->get_head_cache_index(), 'crawler:none' ) === false ) {
				...
				NgfbConfig::set_variable_constants( self::$dnc_const );	// set "do not cache" constants
			}
		}
		...
		public function get_single_mt( $tag, $type, $name, $value, $cmt, array &$mod ) {
			...
			foreach ( $ret as $num => $parts ) {
				...
				if ( SucomUtil::get_const( 'NGFB_FILTER_SINGLE_TAGS' ) )
